function [OutputImage,Omega,time,counter,w,alpha] = Gspir_BlocoSemIteracao(InputImage, Omega, nNeighbors, winsize, winLarge, uband, numit)
...
hold on
Contour = GetContours(Omega,0);   %contorno da região degradada.
OmegaAux = Omega;
% TESTE = OutputImage;  %teste visual
% TESTE8=TESTE; AUX=TESTE8(:,:,1); AUX(Contour)=0; TESTE8(:,:,1) = AUX; AUX=TESTE8(:,:,2); AUX(Contour)=0; TESTE8(:,:,2) = AUX; AUX=TESTE8(:,:,3); AUX(Contour)=0; TESTE8(:,:,3) = AUX;
% OutputImage = TESTE8;
%figure(12),imshow(uint8(TESTE8)); title('Priorities p'); %xlim([135 161]); ylim([206 232]);
%figure(13), imshow(mat2gray(OutputImage));
%figure(1)
%pause
%-----------------------------------------------------------------------
while (nInpainted < nInpaint) && (counter ~= numit) %&& nInpainted <= 3000
...
%---------------------------------------------------------------
  ...
  %Priorities = C(isConf).*D(isConf)      %equação 9 do paper ou eq 32 samara (prioridades calculadas dos termos da borda)
  %Priorities = R(isConf);
...
%     TESTE6(p) = 255;
  ...
  %BinaryMaskAux = ~OmegaAux(iRange, jRange);
  
  %figure(2), imshow(mat2gray(Template))
  
  HpLarge = GetPatch([sizeRows sizeCols], winLarge, p);  %Região de amostra (quadrado maior)
  OmegaSquare = OmegaAux;
  OmegaSquare(HpLarge) = ones(size(HpLarge));            
  DynamicSample = ~(OmegaSquare == OmegaAux);                %Região da amostra na mascara.
  %figure(2), imshow(mat2gray(DynamicSample))
...
%---------------------------Kernel------------------------------------ 
Hr = Kernel(OutputImage,winsize,h,r, nNeighbors,Hq,V);
...
%-------------------------------------------------------------------
  ...
  %figure(9), imshow(mat2gray(OutputImage));
  %--------------------------Teste Visual----------------------------- 
...
time = toc;
imwrite(mat2gray(OutputImage), 'GSPIRBlocoNoIter_output.png')
%==========================================================================
end %end function
...
%R = 1;
   ...
   for j=1:nNeighbors       
        W(j)= (1/(pi*(h^2)))*(exp(-((R(j))/(1))^2))+ 0.00001;
   end
