function [OutputImage] = Gspir_BlocoTexture(InputImage, Omega, nNeighbors, winsize, winLarge, uband, numit)
...
%    figure(7),imshow(uint8(TESTE6)); title('Priorities p'); %xlim([135 161]); ylim([206 232]);
  ...
  %[SSDBinaryList, SSD, ChosenIdx] = SSDCompv2(Template, BinaryMask, gaussM, Patches, nNeighbors);            
  ...
  h = a*max(r);
  BestPatches = Patches(:,ChosenIdx,:);                            %seleciona os nNeighbors melhores patches.
...
%-------------------------------------------------------------------
  ...
  OutputImage = UpdateImage2(OutputImage,Hp,Hr,toFill);
...
time = toc;
imwrite(OutputImage, 'GSPIRBloco_output.png')
%==========================================================================
end %end function
...
R = 1;
   ...
   for j=1:nNeighbors       
        W(j)= (1/(pi*(h^2)))*(exp(-((R)/(1))^2))+ 0.00001;
   end
